    ...
    <div class="m-7 shadow-xl w-full bg-gray-100 flex flex-col">
      <form class="m-5 w-4/12" @submit.prevent="updateData">
        <div class="flex flex-col gap-4 px-0 py-2">
          ...
          <label
            class="w-64 flex flex-col items-center px-4 py-6 bg-gray-100 rounded-md shadow-md tracking-wide uppercase border border-blue cursor-pointer hover:bg-purple-600 hover:text-white text-purple-600 ease-linear transition-all duration-150"
          >
            ...
            <span v-if="bio.avatar" class="mt-2 text-base leading-normal">{{ bio.avatar.name }} </span>
            <input ref="avatarInput" accept=".png" class="hidden" required type="file" @change="previewFiles" />
          </label>
          <div>
            <label class="text-gray-700">Small bio</label>
            <input v-model="bio.bioSmall" class="py-2 pl-2 border border-gray-200 w-full bg-gray-200 text-gray-700 focus:border-gray-100" placeholder="Full time toast enjoyer" type="text" />
          </div>
          <div>
          ...
          <div class="w-full flex flex-row gap-2">
            <button
              v-if="!uploading"
              class="p-2 border border-indigo-500 hover:bg-indigo-500 hover:text-white duration-100 ease-in-out w-2/12 text-indigo-500 flex flex-row justify-center items-center gap-1"
              type="submit"
            >
              <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                <path d="M11 16l-4-4m0 0l4-4m-4 4h14m-5 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h7a3 3 0 013 3v1" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" />
              </svg>
              Send
            </button>
            <div v-if="uploading">
        ...
        <div class="flex flex-col gap-4 px-0 py-2">
          ...
          <div v-if="invalid2" class="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded relative" role="alert">
            ...
            <span class="absolute top-0 bottom-0 right-0 px-4 py-3">
              <svg class="fill-current h-6 w-6 text-red-500" role="button" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg" @click="invalid2 = false">
                <title>Close</title>
                <path
          ...
          <div>
            ...
            <svg class="font-medium text-2xl text-gray-600 absolute p-2.5 px-3 w-11" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
            ...
            <input v-model="info.username" class="py-2 pl-10 border border-gray-200 w-full bg-gray-200 text-gray-700 focus:border-gray-100" placeholder="Username" type="text" />
          </div>
          <div>
            ...
            <input v-model="info.email" class="py-2 pl-10 border border-gray-200 w-full bg-gray-200 text-gray-700 focus:border-gray-100" placeholder="Email address" type="email" />
          </div>
          <div>
            ...
            <input v-model="info.password_old" class="py-2 pl-10 border border-gray-200 w-full bg-gray-200 text-gray-700 focus:border-gray-100" placeholder="Password" type="password" />
          </div>
          <div>
            ...
            <input v-model="info.password_new" class="py-2 pl-10 border border-gray-200 w-full bg-gray-200 text-gray-700 focus:border-gray-100" placeholder="Password" type="password" />
          </div>
          <div class="w-full flex flex-row gap-2">
            <button
              class="p-2 border border-indigo-500 hover:bg-indigo-500 hover:text-white duration-100 ease-in-out w-2/12 text-indigo-500 flex flex-row justify-center items-center gap-1"
