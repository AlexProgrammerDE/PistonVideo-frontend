      ...
      <div class="px-6 py-3">
        ...
        <div class="py-1 pt-1 pb-2">
          <span v-for="tag in video.tags" :key="tag"
                class="inline-block bg-gray-200 rounded-full px-2 py-1 text-sm font-semibold text-gray-700 mr-1 mb-1">#{{ tag
            }}</span>
        </div>
        <div class="my-3 flex flex-row w-full bg-gray-300 w-24 items-center p-6 space-x-6 rounded-xl shadow-lg">
          ...
          <a v-bind:href="'/user?id=' + video.uploader.id"
             class="inline-block text-gray-700 text-lg py-1">{{ video.uploader.username }}</a>
        </div>
      </div>
    </div>
...
export default {
  ...
  async asyncData({ query, $axios }) {
    ...
    return { video: response.data, description: new MarkdownIt().render(response.data.description) };
  },
  head() {
    return {
