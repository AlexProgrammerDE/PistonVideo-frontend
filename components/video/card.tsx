    ...
    <div className='p-2'>
      <div className='w-64 h-auto rounded overflow-hidden shadow-lg bg-gray-100'>
        ...
        <div className='px-6 py-3'>
          <div className='flex justify-center md:justify-end -mt-12'>
            <a href={'/user?id=' + video.uploader.id}>
              <Image className='w-14 h-14 object-cover rounded-full border-2 border-indigo-500'
        ...
        <div className='px-6 pt-1 pb-2'>
          {video.tags.map(tag => (
              <span className='inline-block bg-gray-200 rounded-full px-2 py-1 text-xs font-semibold text-gray-700 mr-1 mb-1'>#{tag}</span>))}
        </div>
      </div>
    </div>
  );
